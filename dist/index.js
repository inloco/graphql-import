...
function parseImportLine(importLine) {
    ...
    var matches = importLine.match(/^import\s+(\*|(.*))\s+from\s+('|")(.*)('|");?$/);
...
function importSchema(schema, schemas) {
    ...
    var secondFirstTypes = typeDefinitions[0].filter(function (d) { return !lodash_1.includes(rootFields, d.name.value); });
    var otherFirstTypes = lodash_1.flatten(typeDefinitions.slice(1)).filter(function (d) { return !lodash_1.includes(rootFields, d.name.value); });
    var firstSet = firstTypes.concat(secondFirstTypes, otherFirstTypes);
    ...
    var _loop_1 = function (type) {
        if (!lodash_1.includes(processedTypeNames, type.name.value)) {
            ...
            existingType.fields = lodash_1.uniqBy(existingType.fields.concat(type.fields), 'name.value');
        }
    };
    ...
    document = __assign(__assign({}, document), { definitions: definition_1.completeDefinitionPool(lodash_1.flatten(allDefinitions), firstSet, lodash_1.flatten(typeDefinitions)) });
...
function collectDefinitions(imports, sdl, filePath, schemas, processedFiles, typeDefinitions, allDefinitions) {
    ...
    var key = isGraphQLFile(filePath) ? path.resolve(filePath) : filePath;
